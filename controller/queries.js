...
const queryFind = function(queryParams = {}) {
  ...
  return Product.findAll(queryOptions).then(products => {
...
const genFilterArray = function(filterObj, andArray) {
  ...
  if (filterObj.categoryId)
    andArray.push({CategoryId: filterObj.categoryId});
  if (filterObj.minPrice.length > 0)
    andArray.push({
      price: {
        $gte: parseInt(filterObj.minPrice)
      }
    });
  if (filterObj.maxPrice.length > 0)
    andArray.push({
      price: {
        $lte: parseInt(filterObj.maxPrice)
      }
    });
  }
...
const genCartItem = function(productId) {
  return Product.findById(productId, {include: models.Category}).then((product) => {
    return {
