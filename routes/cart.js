...
router.post("/charges", (req, res) => {
  ...
  stripe.charges
    ...
    .then(() => {
      var promiseProductArr = [];
      var promiseUpdateArr = [];
      shoppingCart.forEach(product => {
        promiseProductArr.push(Product.find({ where: { id: product.id } }));
      });

      Promise.all(promiseProductArr).then(results => {
        results.forEach(foundProduct => {
          let newCount = Number(foundProduct.unitsSold) +
            Number(product.quantity);
          promiseUpdateArr.push(
            Product.update(
              { where: { id: product.id } },
              { unitsSold: newCount }
            )
        ...
        Promise.all(promiseUpdateArr).then(() => {
          req.session.shoppingCart = [];
          res.redirect("/cart");
        });
      });
